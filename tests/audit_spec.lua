...
describe("repeatable surfaces", function()
  it("can name the same buffer twice without E95", function()
    -- Opening the help tab, the profile or the workspace a second time threw
    -- "Buffer with this name already exists" and the surface never appeared.
    local util = require("review.util")
    local a = vim.api.nvim_create_buf(false, true)
    assert.is_true(util.name_buffer(a, "review://test-surface"))
    local b = vim.api.nvim_create_buf(false, true)
    assert.is_true(util.name_buffer(b, "review://test-surface"))
    assert.equals("review://test-surface", vim.api.nvim_buf_get_name(b))
  ...
  it("is a no-op when the buffer already holds the name", function()
    ...
    assert.is_true(util.name_buffer(buf, "review://same"))
    assert.is_true(util.name_buffer(buf, "review://same"))
    assert.is_true(vim.api.nvim_buf_is_valid(buf))
  end)

  it("opens the help tab twice", function()
    local help = require("review.ui.help")
    help.open()
    local first = vim.api.nvim_get_current_buf()
    assert.equals("review://help", vim.api.nvim_buf_get_name(first))
    help.open()
    assert.equals("review://help", vim.api.nvim_buf_get_name(vim.api.nvim_get_current_buf()))
  end)

  it("opens the performance report twice", function()
    local perf = require("review.perf")
    perf.open()
    perf.open()
