...
---@param file string
---@param side string
function M.render(bufnr, store, file, side)
  ...
  for _, root in ipairs(store:threads_for_file(file)) do
    ...
    if root.side == side and not root.hidden and (root.line_start or 1) <= line_count then
      local lnum = math.max(1, root.line_start or 1)
...
function M.toggle_at_cursor(bufnr, store, file, side)
...
---@param side string
---@return table|nil
function M.thread_at_cursor(store, file, side)
