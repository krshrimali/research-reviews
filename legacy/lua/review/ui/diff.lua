...
---@return table|nil
function M.context()
  return M.buffer_context(vim.api.nvim_get_current_buf())
end
...
function M.refresh_markers(store)
  ...
  for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
    ...
    if ctx and ctx.file then
      markers.render(ctx.bufnr, store, ctx.file, ctx.side)
    end
  end
end
...
function M.add_comment(store, opts)
  ...
  if vstart > 0 and vend > 0 and vend >= vstart and vim.fn.mode():match("[vV]") == nil then
    line_start, line_end = vstart, vend
  else
    local cur = vim.api.nvim_win_get_cursor(0)[1]
    line_start, line_end = cur, cur
  end

  -- Seed the composer with the selected lines.
  local seed = vim.api.nvim_buf_get_lines(ctx.bufnr, line_start - 1, line_end, false)
